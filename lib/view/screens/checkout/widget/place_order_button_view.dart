...
class _PlaceOrderButtonViewState extends State<PlaceOrderButtonView> {
              ...
              onPressed: () {
                ...
                if (!widget.selfPickUp! && orderProvider.addressIndex == -1) {
                  ...
                  for (int index = 0; index < cartList.length; index++) {
                    Cart cart = Cart(
                      productId: cartList[index].id, price: cartList[index].price!+getAllAdonPrice(cartList[index]),
                      discountAmount: cartList[index].discountedPrice,
                      quantity: cartList[index].quantity.toString(),
                      taxAmount: cartList[index].tax,
                      addOnIds: getAddonIds(cartList[index]),
                      addOnQty: getAddonQty(cartList[index]),
                      variant: '', variation: [Variation(type: cartList[index].variation != null ? cartList[index].variation!.type : null)],
                    );
                    // imageData.addAll(cartList[index].imageList!);
                    // widget.images!.addAll(cartList[index].imageList!);
                    carts.add(cart);
                  }
                  ...
                  PlaceOrderBody placeOrderBody = PlaceOrderBody(
                    ...
                    imagesList: widget.images!.isNotEmpty ? widget.images:[],
                    ordermessage: getAllMessage(cartList)+widget.message.text
                  );
  ...
  String getAllMessage(List<CartModel> cartList) {

    String value ="";
    for(int i=0; i<cartList.length; i++)
      {
        if(cartList[i].message!.isNotEmpty)
          {
            value = "${cartList[i].message!}\n";
          }
      }
    return value;
  }

  List<String> getAddonIds(CartModel cartList) {

    List<String>data = [];

    if(cartList.addOn!.isNotEmpty)
      {
        for(int i=0; i<cartList.addOn!.length; i++)
          {
            data.add(cartList.addOn![i].id.toString());
          }
      }
  ...
  List<String> getAddonQty(CartModel cartList) {
    ...
    if(cartList.addOn!.isNotEmpty)
    {
      for(int i=0; i<cartList.addOn!.length; i++)
      {
        data.add(cartList.addOnQty![i].toString());
      }
    }
