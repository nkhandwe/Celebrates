...
class _CartScreenState extends State<CartScreen> {
            ...
            return Consumer<CartProvider>(
              builder: (context, cart, child) {
                ...
                for (var cartModel in cart.cartList) {
                  ...
                  addonPrice += getAllAdonPrice(cartModel);
                  discount = discount + (cartModel.discount! * cartModel.quantity!);
                  tax = tax + (cartModel.tax! * cartModel.quantity!);
                }

                double subTotal = itemPrice+addonPrice + (configModel.isVatTexInclude! ? 0 : tax);
                ...
                Column(children: [
                      ...
                      CartButtonView(
                        ...
                        image: getImage(cart.cartList),
                        message: getMessage(cart.cartList),
                        itemPrice: itemPrice,
                        imageFile: getAllImage(cart.cartList)!=null ? getAllImage(cart.cartList) : [],
                        total: total,
                            ...
                            children: [
                              ...
                              Expanded(child: Column(mainAxisAlignment: MainAxisAlignment.start, children: [
                                ...
                                CartButtonView(
                                  ...
                                  imageFile: getAllImage(cart.cartList).isNotEmpty ? getAllImage(cart.cartList) : [],
                                  isFreeDelivery: isFreeDelivery,
                                ),
                              ]))
  ...
  bool getImage(List<CartModel> cartList) {
    bool value = false;

    for(int i=0; i<cartList.length; i++)
      {
        if(cartList[i].product!.imgStatus=="1")
          {
            value = true;
          }
      }
  ...
  List<XFile>getAllImage(List<CartModel>cartList)
  {
    List<XFile>value = [];

    for(int i=0; i<cartList.length; i++)
    {
      if(cartList[i].imageList!=null)
      {

        value.addAll(cartList[i].imageList!);
      }
    }
    return value;
  }

  bool getMessage(List<CartModel> cartList) {
    bool value = false;

    for(int i=0; i<cartList.length; i++)
      {
        print("dferfeqr${cartList[i].product!.msgStatus}");
        if(cartList[i].product!.msgStatus=="1")
          {
            value = true;
          }
      }
  ...
  double getAllAdonPrice(CartModel cartModel) {

    double price = 0;

    if(cartModel.addOn!=null)
      {
        for(int i=0; i<cartModel.addOn!.length; i++)
          {
            price += double.parse(cartModel.addOn![i].price.toString())*double.parse(cartModel.addOnQty![i].toString());
          }
      }
    return price;
  }
...
class CartButtonView extends StatelessWidget {
  ...
  }) : _subTotal = subTotal, _configModel = configModel, _isFreeDelivery = isFreeDelivery, _itemPrice = itemPrice,
        _image = image,
  _message = message,
  _imageFile=imageFile,
        _total = total, super(key: key);
  ...
  final List<XFile>_imageFile;

  @override
  Widget build(BuildContext context) {

    print("dfsfssfg${_image}");
    print("dfsfssfg${_message}");
    print("dfsfssfg${_imageFile.length}");
