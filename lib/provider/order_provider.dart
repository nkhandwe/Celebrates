...
OrderProvider extends ChangeNotifier {
  ...
  List<AddOns>_addonItem = [];
  List<dynamic>_addonQTY = [];
  List<dynamic>_addonQTY2 = [];
  ...
  List<AddOns>? get addonItem => _addonItem;
  List<dynamic>? get addonQTY => _addonQTY;
  List<dynamic>? get addonQTY2 => _addonQTY2;
    ...
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      ...
      for (var element in _orderDetails!) {
        try{
          ...
          _addonQTY=jsonDecode(element.addOnIds!);
          _addonQTY2 = jsonDecode(element.addOnQtys!);
          print("addonItem=>${jsonDecode(element.addOnIds!)}");
          print("addonItem=>${_addonQTY}");
          print("addonItem=>$_addonQTY2");
  ...
  Future<void> placeOrder(PlaceOrderBody placeOrderBody, Function callback) async {
    ...
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      ...
      ApiResponse apiResponse2 = await orderRepo!.placeOrderWith(placeOrderBody.imagesList!,placeOrderBody.ordermessage!,orderID);
      ...
      {
        callback(false, ApiChecker.getError(apiResponse2).errors![0].message, '-1');
      }
